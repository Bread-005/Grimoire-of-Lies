...
import {characterTypeDistribution, outsiderRoles, players} from "../../../roleSelection.js";
...
function librarianInfo(player) {
    ...
    if (getsFalseInfo(player)) {
        ...
        if (outsiderBluffPlayers.length === 0) {
            if (characterTypeDistribution[storage.playerCount][1] === 0 && players.filter(p => p.role.characterType === "Outsider").length > 0 && Math.random() - 0.5 > 0.66) {
                player.info += "0 Outsiders in-play";
            }
            const list = [];
